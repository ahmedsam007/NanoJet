...
struct NanoJetApp: App {
    ...
    var body: some Scene {
        Window("NanoJet", id: "main-window") {
            ContentView()
                .environmentObject(appModel)
                .onOpenURL { url in
                    // Expect: nanojet://add?url=<encoded>&headers=<base64(json)>
                    guard url.scheme == "nanojet" else { return }
        ...
        .commands {
            ...
            CommandMenu("Tools") {
                ...
                Divider()
                Button {
                    if let w = NSApp.windows.first(where: { $0.title == "NanoJet" }) {
...
private extension View {
    func asNSAttributedString() -> NSAttributedString {
        ...
        return att ?? NSAttributedString(string: "NanoJet")
    }
}
